...
public class EmployeeController {
	...
	public Employee getEmployeeById(@PathVariable Long id) {
		...
		return ejr.findById(id).orElseThrow(() -> new EmployeeNotFoundException("Employee not found with id " + id));
	}

	@PutMapping("/{id}")
	public Employee updateEmployee(@PathVariable Long id, @RequestBody Employee e) {

		Employee emp = ejr.findById(id).orElseThrow(() -> new EmployeeNotFoundException("Employee not found"));
	...
	public List<Employee> sortBySalary() {
		...
		return ejr.findAll(Sort.by("salary").descending());
	}
