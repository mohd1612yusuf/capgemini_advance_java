...
public class AppointmentToPrescriptionService {
	...
	public void appointmentToPrescription() {
		...
		Prescription p = new Prescription("Paracetamol", "1 tablet twice daily", LocalDate.now(), true);
		Appointment a1 = new Appointment(LocalDateTime.now(), "SCHEDULED", "Fever");
		a1.setPrescription(p);
		em.persist(a1);
		Appointment a2 = new Appointment(LocalDateTime.now().plusHours(1), "COMPLETED", "General Checkup");
	...
	private void printAppointment(Appointment appt) {
		...
		if (appt.getPrescription() != null) {
			System.out.println("Medicines: " + appt.getPrescription().getMedicines());
		} else {
