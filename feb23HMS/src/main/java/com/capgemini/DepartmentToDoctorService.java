...
public class DepartmentToDoctorService {
	...
	public void navigationTest(long deptId, long doctorId) {
		...
		System.out.println("\nDoctor belongs to: " + doc.getDepartment().getName());
	}
	...
	public void findDoctorsByDepartment(String name) {
		List<Doctor> list = em.createQuery("SELECT d FROM Doctor d WHERE d.department.name = :dname", Doctor.class)
				.setParameter("dname", name).getResultList();
		System.out.println("\nDoctors in " + name + ":");
		for (Doctor d : list) {
			System.out.println(d.getName());
		}
