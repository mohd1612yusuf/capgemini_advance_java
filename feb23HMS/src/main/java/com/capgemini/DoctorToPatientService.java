...
public class DoctorToPatientService {
	...
	public void patientsOfDoctor(String name) {
		Doctor d = em.createQuery("SELECT d FROM Doctor d JOIN FETCH d.patients WHERE d.name=:n", Doctor.class)
	...
	public void doctorsOfPatient(String pname) {
		List<Doctor> list = em.createQuery("SELECT d FROM Doctor d JOIN d.patients p WHERE p.name=:n", Doctor.class)
				.setParameter("n", pname).getResultList();
